...
export function EnvVarsScreen(): React.ReactElement {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const list = Object.entries(vars).map(([name, value]) => ({ name, value }));
      setEnvVarList(list);
    } catch (error) {
      setEnvVarList([]);
    }
  ...
  const renderDetail = () => {
    ...
    return (
      <Box flexDirection="column">
        <Text bold color="cyan">{selectedVar.name}</Text>
        <Box marginTop={1}>
          <Text color="gray">Value: </Text>
          <Text>{selectedVar.value.length > 50 ? selectedVar.value.slice(0, 50) + '...' : selectedVar.value}</Text>
        </Box>
        <Box marginTop={2} flexDirection="column">
          <Box>
            <Text backgroundColor="magenta" color="white">{' '}Enter{' '}</Text>
            <Text color="gray"> Edit value</Text>
          </Box>
          <Box marginTop={1}>
            <Text backgroundColor="red" color="white">{' '}d{' '}</Text>
  ...
  const renderListItem = (envVar: EnvVar, _idx: number, isSelected: boolean) => {
    const masked = envVar.value.length > 20 ? envVar.value.slice(0, 20) + '...' : envVar.value;
    return isSelected ? (
