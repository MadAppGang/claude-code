...
function GlobalKeyHandler({ onDebugToggle }: { onDebugToggle: () => void }): null {
  const { state } = useApp();
  ...
  useInput((input, key) => {
    ...
    const isTopLevel = ['plugins', 'mcp', 'mcp-registry', 'statusline', 'env-vars', 'cli-tools'].includes(
    ...
    if (isTopLevel) {
      ...
      if (key.tab) {
        const screens: Screen[] = ['plugins', 'mcp', 'statusline', 'env-vars', 'cli-tools'];
        const currentIndex = screens.indexOf(state.currentRoute.screen as Screen);
        if (currentIndex !== -1) {
...
function ProgressIndicator({ message, current, total }: ProgressIndicatorProps): React.ReactElement {
      ...
      {showDebug && (
        <Box paddingX={1}>
          <Text color="yellow" dimColor>
            DEBUG: {dimensions.terminalWidth}x{dimensions.terminalHeight} | content={dimensions.contentHeight} | screen={state.currentRoute.screen}
          </Text>
        </Box>
      )}
      {progress && <ProgressIndicator {...progress} />}
      <Box flexDirection="column" height={dimensions.contentHeight} paddingX={1} overflow="hidden">
        <Router />
      </Box>
