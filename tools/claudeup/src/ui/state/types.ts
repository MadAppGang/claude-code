import type { Marketplace, McpServer, StatusLineConfig } from '../../types/index.js';
...
export type Screen = 'plugins' | 'mcp' | 'mcp-registry' | 'statusline' | 'env-vars' | 'cli-tools' | 'model-selector';
...
export type AppAction =
  ...
  | { type: 'PLUGINS_DATA_SUCCESS'; marketplaces: Marketplace[]; plugins: PluginInfo[] }
  | { type: 'PLUGINS_DATA_ERROR'; error: Error }

  // MCP screen
  | { type: 'MCP_SELECT'; index: number }
  | { type: 'MCP_DATA_LOADING' }
  | { type: 'MCP_DATA_SUCCESS'; servers: McpServer[]; installedServers: Record<string, boolean> }
  ...
  | { type: 'STATUSLINE_DATA_SUCCESS'; presets: StatusLineConfig[]; currentPreset: string | null }
