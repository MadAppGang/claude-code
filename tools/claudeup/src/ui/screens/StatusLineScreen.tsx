...
export function StatusLineScreen(): React.ReactElement {
  ...
  const [projectStatusLine, setProjectStatusLineState] = useState<string | undefined>();
  const [globalStatusLine, setGlobalStatusLineState] = useState<string | undefined>();
  ...
  const getCurrentStatusLine = (): string | undefined => {
    return statusLine.scope === 'project' ? projectStatusLine : globalStatusLine;
  };
  ...
  const renderPreview = () => {
    ...
    if (selectedItem.isCustom) {
      return (
        <Box flexDirection="column">
          <Text bold color="cyan">✨ Custom Status Line</Text>
          <Text color="gray">Create your own unique status line!</Text>

          <Box marginTop={1} flexDirection="column">
            <Text bold color="yellow">Available variables:</Text>
            <Box marginTop={1} flexDirection="column">
              <Text><Text color="green" bold>{'{model}'}</Text>          <Text color="gray">→</Text> Model name</Text>
              <Text><Text color="green" bold>{'{model_short}'}</Text>    <Text color="gray">→</Text> Short name</Text>
              <Text><Text color="yellow" bold>{'{cost}'}</Text>           <Text color="gray">→</Text> Session cost</Text>
              <Text><Text color="blue" bold>{'{cwd}'}</Text>            <Text color="gray">→</Text> Working directory</Text>
              <Text><Text color="magenta" bold>{'{git_branch}'}</Text>    <Text color="gray">→</Text> Git branch</Text>
              <Text><Text color="cyan" bold>{'{input_tokens}'}</Text>   <Text color="gray">→</Text> Input tokens</Text>
              <Text><Text color="cyan" bold>{'{output_tokens}'}</Text>  <Text color="gray">→</Text> Output tokens</Text>
              <Text><Text color="red" bold>{'{session_duration}'}</Text><Text color="gray">→</Text> Duration</Text>
            </Box>
          </Box>
        </Box>
      );
    ...
    if (selectedItem.preset) {
      ...
      return (
        <Box flexDirection="column">
          <Text bold color="cyan">◆ {selectedItem.preset.name}</Text>
          ...
          <Box marginTop={1} flexDirection="column">
            <Text color="gray" dimColor>Template:</Text>
  ...
  const statusContent = (
    <>
      ...
      <Text color="green">{currentValue ? currentValue.slice(0, 35) + (currentValue.length > 35 ? '...' : '') : '(not set)'}</Text>
    </>
  );
